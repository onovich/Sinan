# Sinan Engine Development Plan

## Project Goal

Sinan Engine is an AI-native, text-data-driven Web 3D game engine and project-specific editor. Its long-term value is not a generic Unity or Godot clone; it is a maintainable engine stack where TypeScript, JSON, schemas, validators, registries, adapters, and tests make runtime, renderer, physics, assets, input, UI, director, and editor semantics editable by humans and AI agents through Git-friendly files.

The original Sinan Scene Director scope is now the engine's first-party Director System. Events, conditions, actions, timelines, camera shots, animation cues, and cinematic flow remain central, but they are no longer treated as a standalone product outside the engine runtime.

The first runtime target is Three.js, with a deliberate adapter boundary so the game semantics can later survive a Babylon.js runtime migration.

## Non-Negotiable Architecture Rules

1. `data/**/*.json` is the source of truth for assets, prefabs, levels, events, timelines, camera shots, render styles, physics settings, input maps, UI flows, and other game semantics.
2. `src/runtime/three/**` owns Three.js. Game, world, events, director, schemas, data, migrations, and future renderer/physics/input/UI semantic modules must stay adapter-neutral.
3. React owns editor shell, HUD, panels, selection, inspector tabs, and other slow UI state. Per-frame game state does not live in React state.
4. Every JSON input needs a Zod schema and clear validation errors.
5. Actions and conditions execute through registries. JSON must not contain `eval`, arbitrary script strings, or unregistered function calls.
6. Timeline scrub must not run irreversible or runtime-only side effects.
7. Editor mutations go through command objects so undo, redo, save, and test coverage remain tractable.

## Phase Plan

### Phase 0: Project Foundation

Goal: a runnable Vite + React + TypeScript app with a visible Three.js canvas.

Work:

- Initialize package tooling and scripts.
- Install `three`, `zod`, `vitest`, and Playwright.
- Add ESLint/Prettier, including import boundary checks.
- Create the recommended `src`, `data`, `public`, `scripts`, and `tests` structure.
- Implement the minimal `WebRuntime` interface and `ThreeRuntime` init/render/dispose path.

Acceptance:

- `npm run dev` starts the editor shell.
- `npm run build`, `npm run test`, and the first lint/typecheck checks pass.
- The first page renders a nonblank Three.js scene.

### Phase 1: Schema And Data Loading

Goal: load project data from JSON and instantiate a level.

Work:

- Implement common, transform, entity, prefab, level, and asset manifest schemas.
- Add `DataRepository`, JSON loading, and project validation.
- Add demo `assets.manifest.json`, prefabs, and `level_01.json`.
- Implement model loading and entity-to-runtime-object instantiation.
- Add `scripts/validate-data.ts` and cross-reference checks.

Acceptance:

- `level_01.json` loads into the scene.
- Invalid JSON produces actionable schema errors.
- Entity transforms are correctly applied and reference errors are caught.

MVP to Post-MVP transition note:

- The Phase 0-7 MVP used deterministic placeholder geometry to keep data, schema, editor, and runtime adapter contracts testable before real assets existed.
- Post-MVP Phase 8 now adds `GLTFLoader` inside `src/runtime/three/**`, preserves the renderer-neutral `WebRuntime` boundary, caches loaded GLB scenes/animations, and keeps placeholder fallback for missing or invalid local assets.
- See `docs/phase-8-real-asset-runtime.md` for current GLB loading behavior, generated development assets, fallback behavior, and remaining limitations.

### Phase 2: Editor MVP

Goal: select, inspect, transform, save, undo, and redo level edits.

Work:

- Build `EditorApp`, `Viewport`, `HierarchyPanel`, and `InspectorPanel`.
- Add picking and TransformControls through the runtime interface.
- Add editor commands for transform and component updates.
- Implement command history.
- Add a dev-only save JSON API restricted to `data/**`.

Acceptance:

- Clicking an entity selects it.
- Move/rotate/scale gizmos update entity transforms.
- Inspector edits and gizmo edits are undoable.
- Saving updates the relevant JSON file.

### Phase 3: Events, Conditions, And Actions

Goal: trigger gameplay/director behavior through data-driven events.

Work:

- Implement event, trigger, condition, and action schemas.
- Add trigger, condition, and action registries.
- Support MVP triggers such as `entity.interact`, `level.start`, and `timeline.finished`.
- Support MVP actions such as `flag.set`, `door.open`, `timeline.play`, and `animation.play`.
- Build an Event Inspector MVP.

Acceptance:

- Interacting with `switch_a` evaluates conditions and dispatches actions.
- Actions are schema-validated and registry-backed.
- Unit tests cover condition evaluation and action dispatch.

### Phase 4: Director And Timeline Runtime

Goal: play JSON timelines with animation, camera, sound/subtitle markers, and safe scrub behavior.

Work:

- Implement timeline schemas and `TimelinePlayer`.
- Support action, animation, camera shot, property, subtitle, and sound tracks.
- Add action side-effect classification.
- Add `timeline.finished` trigger dispatch.
- Add preview-safe scrub behavior.

Acceptance:

- `tl_open_gate` can move camera, play gate animation, play markers, and set flags.
- Scrubbing previews camera/animation without running runtime-only actions.
- Timeline unit tests cover sampling and marker execution.

### Phase 5: Camera Shot System

Goal: author and preview simple cinematic camera shots.

Work:

- Implement `cameraShot` schema, `CameraShotPlayer`, and `DirectorCameraSystem`.
- Add keyframed/static/follow/lookAt MVP shot types.
- Build `CameraShotPanel`.
- Add Set Key From View, View Through Camera, Look At Selected, and preview controls.

Acceptance:

- `cam_gate_reveal` can be created, edited, previewed, and played by a timeline.
- The runtime camera receives poses through the runtime adapter, not direct Three.js access from director code.

### Phase 6: Timeline Editor

Goal: edit timeline tracks and keyframes in the editor.

Work:

- Build `TimelinePanel`, ruler, playhead, track list, and keyframe/action marker editing.
- Support add/remove/move keyframes and tracks.
- Save `timeline.json` through editor commands.

Acceptance:

- Users can author action markers, camera shot tracks, and animation tracks.
- Saved timelines reload cleanly and pass data validation.

### Phase 7: Trigger And Physics Layer

Goal: reliable trigger zones and lightweight collision support.

Work:

- Add collider schemas and trigger zone visualization.
- Implement AABB trigger MVP.
- Add debug draw.
- Keep the MVP on data-backed AABB trigger zones; evaluate Rapier integration only after trigger semantics, validation, and editor visualization are stable.

Acceptance:

- Entering a trigger zone can fire an event.
- Trigger bounds are visible in edit mode.
- Reference validation catches missing trigger targets.

## Immediate Implementation Order

Start with the first file batch from the architecture guide:

1. `src/schemas/common.schema.ts`
2. `src/schemas/transform.schema.ts`
3. `src/schemas/entity.schema.ts`
4. `src/schemas/prefab.schema.ts`
5. `src/schemas/level.schema.ts`
6. `src/runtime/WebRuntime.ts`
7. `src/runtime/RuntimeTypes.ts`
8. `src/runtime/three/ThreeRuntime.ts`
9. `src/data/DataRepository.ts`
10. `src/world/World.ts`
11. `src/editor/EditorApp.tsx`
12. `src/editor/Viewport.tsx`
13. `data/assets.manifest.json`
14. `data/prefabs/*.json`
15. `data/levels/level_01.json`
16. `scripts/validate-data.ts`

## Validation Plan

Phase 0 should establish the permanent validation command set:

```txt
npm run typecheck
npm run lint
npm run build
npm run test
npm run validate-data
```

After those scripts exist, wire them into `.codex/project-ops-workflow.json` and `.codex/project-git-workflow.json` so future commits use the repository wrappers instead of ad hoc commands.

## Post-MVP Plan

Phase 0-7 now define the architecture MVP. Continue with:

- `docs/post-mvp-development-plan.md` for Phase 8-15 scope, acceptance criteria, and estimated session rounds.
- `docs/post-mvp-execution-workflow.md` for the goal-mode execution workflow, phase gates, validation commands, and commit rhythm.

The post-MVP route explicitly prioritizes rendering quality and editor UI/UX maturity. Phase 9 is the demo visual pass, Phase 11 is the editor UI/UX redesign, and Phase 12 carries detailed authoring workflows plus data safety.

After Phase 14, the next product route is the Abeto Messenger-like vertical slice:

- `docs/abeto-messenger-development-plan.md` for Phase 15-22 scope, budgets, acceptance gates, and estimated session rounds.
- `docs/phase-15-abeto-scope-lock-goal-mode-execution-guide.md` for the next goal-mode handoff.

Phase 14 release-candidate status is accepted in `docs/phase-14-release-candidate-finalization.md`. Phase 15 is a scope-lock and handoff phase only; it should not start runtime, UI, shader, gameplay, multiplayer, or asset-compression implementation.

Current route update: Phase 17 is PASS. The active goal-mode guide is `docs/phase-18-shader-glsl-material-runtime-foundation-goal-mode-execution-guide.md`: Shader GLSL Material Runtime Foundation. The executor should read `AGENTS.md`, the main architecture guide, `docs/abeto-messenger-development-plan.md`, `docs/Web3D_Shader_GLSL_MVP_支持度评估与实施计划.md`, `docs/Web3D_Shader_研发方案与架构指南_GLSL_MVP.md`, and the Phase 17 final report before starting Phase 18.

Historical note: Phase 17 from `docs/phase-17-asset-budget-compression-goal-mode-execution-guide.md` prepared asset, texture, and compression prerequisites for the Shader GLSL MVP; actual `MaterialRuntime` and GLSL work starts in Phase 18.

Current shader route update: Phase 21 is PASS. The project now has production shader compile coverage, deterministic visual regression fixtures, structured fallback diagnostics, HMR/precompile guidance, a local low-end Chromium shader baseline, and an integrated shader production quality guard. The next required gate is Phase 21.5: Engine Maturity External Contract Gate. Use `docs/phase-21-5-engine-maturity-external-infrastructure-contract-gate-goal-mode-execution-guide.md` as the active 6-round goal-mode guide before Phase 22 implementation.

External infrastructure route update: before Phase 22 LOD/Instancing starts, run Phase 21.5 from `docs/phase-21-5-engine-maturity-external-infrastructure-contract-gate-goal-mode-execution-guide.md`. Phase 21.5 creates Sinan-owned RFCs, adapter boundary policy, POC briefs, mature dependency evaluation templates, and a compatibility matrix for external infrastructure cooperation. It is a contract gate, not a runtime implementation phase.

Phase 21.5 routing update: Phase 21.5: Engine Maturity And External Infrastructure Contract Gate now routes future POCs through `docs/phase-21-5-poc-sequencing-and-roadmap-routing.md`, `docs/strategy/adapter-boundary-policy.md`, `docs/strategy/partner-poc-brief-template.md`, `docs/strategy/mature-dependency-evaluation-template.md`, and `docs/strategy/adapter-compatibility-matrix-template.md`. Phase 22 remains the next implementation phase after the Phase 21.5 PASS final report and pushed commits.

Phase 21.5 final route update: Phase 21.5 is PASS in `docs/phase-21-5-engine-maturity-external-infrastructure-contract-gate-final-report.md`. The next required implementation phase is Phase 22: LOD, Instancing, And Vegetation from `docs/abeto-messenger-development-plan.md`. Phase 22 should use the Phase 21.5 RFCs and adapter policy as guardrails, not as permission to add runtime dependencies or external adapters.

Phase 22 active route update: use `docs/phase-22-lod-instancing-and-vegetation-goal-mode-execution-guide.md` as the active 12-round goal-mode guide. Phase 22 uses traditional data-driven LOD with preauthored or offline-generated LOD assets, deterministic scatter, Three `InstancedMesh` runtime support, low-end LOD bias, and perf smoke evidence. It must not implement runtime dynamic mesh reduction, Nanite-style virtualized geometry, external asset-pipeline dependencies, or Phase 23 spherical world/gameplay scope.

Phase 22 final route update: Phase 22 is PASS in `docs/phase-22-lod-instancing-and-vegetation-final-report.md`. The next required implementation phase is Phase 23: Compact Spherical World Prototype from `docs/abeto-messenger-development-plan.md`. Phase 23 may use Phase 22 LOD and instancing infrastructure for distant-region readability, but spherical placement, camera behavior, player movement, and region data must remain data-first and must not pull in physics, input, Runtime UI, audio, narrative importers, or external adapters without a scoped guide.

Phase 23 active route update: use `docs/phase-23-compact-spherical-world-prototype-goal-mode-execution-guide.md` as the active 16-round goal-mode guide. Phase 23 adds cube-sphere projection, data-first spherical placement, compact region data, minimal deterministic player surface movement, stable spherical camera behavior, director camera compatibility, and smoke/perf evidence. It must not implement Phase 24 delivery jobs, full Showcase Mode, Physics/Rapier, InputFlow, ViewRig, Runtime UI, Audio, multiplayer, external adapters, or production input/gameplay scope.

Phase 23 final route update: Phase 23 is PASS in `docs/phase-23-compact-spherical-world-prototype-final-report.md`. The next required implementation phase is Phase 24: Delivery Gameplay Showcase from `docs/abeto-messenger-development-plan.md`. Phase 24 should use the compact spherical-world prototype as the playable environment while keeping delivery jobs, route feedback, player interaction, Runtime UI, audio, and any new input semantics scoped by the Phase 24 guide.

Phase 24 active route update: use `docs/phase-24-delivery-gameplay-showcase-goal-mode-execution-guide.md` as the active 16-round goal-mode guide. Phase 24 adds Showcase Mode without editor panels, a minimal first-party player controller, delivery job data/state, interaction radius, route and target feedback, completion feedback, editor job affordances, and browser smoke for a successful single-player job flow. It must not implement Phase 25 multiplayer-lite, WebSocket rooms, remote avatars, Physics/Rapier, external InputFlow, ViewRig, LudoWeave, Inscape, production Runtime UI framework, Audio runtime, external adapters, or broad gameplay framework scope.

Phase 24 final route update: Phase 24 is PASS in `docs/phase-24-delivery-gameplay-showcase-final-report.md`. The recommended next full-route implementation phase is Phase 25: Multiplayer-lite Social Layer from `docs/abeto-messenger-development-plan.md`. If multiplayer-lite is intentionally skipped, record the skip decision and use Phase 26: Vertical Slice RC Hardening as the next core single-player route.

Phase 25 active route update: use `docs/phase-25-multiplayer-lite-social-layer-goal-mode-execution-guide.md` as the active 16-round goal-mode guide. Phase 25 adds local remote-player simulation first, avatar/emote/stamp/network message schemas, renderer-neutral social state, remote avatar and 3D stamp feedback, and a small local WebSocket room prototype. It must preserve Phase 24 Showcase and delivery smoke, keep WebSocket transport behind adapter or smoke tooling boundaries, and must not implement production networking, auth, persistence, text/voice chat, Physics/Rapier, external InputFlow, ViewRig, LudoWeave, Inscape, production Runtime UI framework, Audio runtime, or unrelated external adapter scope.

Phase 25 final route update: Phase 25 is PASS in `docs/phase-25-multiplayer-lite-social-layer-final-report.md`. The recommended next implementation phase is Phase 26: Vertical Slice RC Hardening from `docs/abeto-messenger-development-plan.md`. Phase 26 should preserve Showcase Mode, delivery smoke, social simulator/WebSocket smoke, low-end budgets, and the data-first runtime boundaries while preparing release-candidate documentation and reproducible validation.

Phase 26 active route update: use `docs/phase-26-vertical-slice-rc-hardening-goal-mode-execution-guide.md` as the active 10-round goal-mode guide. Phase 26 hardens the vertical slice release candidate through reproducible validation, smoke, perf/budget reporting, low-end/mobile evidence, README/developer/release docs, and fresh-checkout evidence where practical. It must not implement production backend, auth, persistence, text/voice chat, new gameplay systems, Physics/Rapier, external InputFlow/ViewRig/LudoWeave/Inscape adapters, production Runtime UI framework, Audio runtime, or unrelated external adapter scope.

Phase 26 final route update: Phase 26 is PASS in `docs/phase-26-vertical-slice-rc-hardening-final-report.md`. The current vertical slice should be held as a local release-candidate baseline. The next route should be a separately scoped planning decision before any production backend, auth, persistence, text/voice chat, Physics/Rapier, external InputFlow/ViewRig/LudoWeave/Inscape integration, production Runtime UI, Audio runtime, deployment, or broad engine expansion starts.

Phase 26.5 active route update: use `docs/phase-26-5-rc-release-packaging-and-baseline-tagging-goal-mode-execution-guide.md` as the active 6-round goal-mode guide. Phase 26.5 packages the Phase 26 local RC baseline with release notes, demo script, validation evidence index, tag policy, and the internal annotated tag `vertical-slice-rc-2026-06-22`. It must not implement product features, production backend/auth/persistence, deployment automation, text/voice chat, mobile input, Physics/Rapier, external adapters, Runtime UI, Audio runtime, or broad engine expansion.

Phase 26.5 final route update: Phase 26.5 is PASS in `docs/phase-26-5-rc-release-packaging-and-baseline-tagging-final-report.md`. The local vertical-slice RC is packaged with release notes, a local demo script, validation evidence index, tag policy, integrated gate evidence, and internal annotated tag `vertical-slice-rc-2026-06-22`. The next product line is not automatic; choose mobile/input experience or content expansion through a separate scoped guide, and keep production backend/auth/persistence/text/voice chat/Physics/Rapier/external adapters/Runtime UI/Audio/deployment out of scope until explicitly planned.
