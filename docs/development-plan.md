...
Current shader route update: Phase 20 is PASS. The project now has renderer-neutral shader globals, engine/runtime global routing, global uniform binding, second production material `story.hologram-scanline`, material lifecycle/resource diagnostics, Three postprocessing runtime, a controlled vignette pass, and public postprocess effect contract `cinematic.vignette`. The next implementation phase is Phase 21: Shader Production Quality Gate. Use `docs/phase-21-shader-production-quality-gate-goal-mode-execution-guide.md` as the active goal-mode guide before implementation.

External infrastructure route update: before Phase 22 LOD/Instancing starts after Phase 21, run Phase 21.5 from `docs/phase-21-5-engine-maturity-external-infrastructure-contract-gate-goal-mode-execution-guide.md`. Phase 21.5 creates Sinan-owned RFCs, adapter boundary policy, POC briefs, mature dependency evaluation templates, and a compatibility matrix for external infrastructure cooperation. It is a contract gate, not a runtime implementation phase.
